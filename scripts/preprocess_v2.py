"""
Stage 1 (v2): Data Collection & Preprocessing - using PyMuPDF for correct Urdu text extraction.
Extracts text from PDFs, detects language per page, cleans it, and saves as structured JSON.
"""

import fitz  # PyMuPDF
import json
import re
from pathlib import Path
from langdetect import detect, DetectorFactory

DetectorFactory.seed = 0

RAW_DIR = Path("data/raw")
OUT_DIR = Path("data/processed")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# Files known to be scanned images - PyMuPDF/pdfplumber can't extract these,
# they're handled separately by ocr_fix.py
SKIP_NATIVE_EXTRACTION = [
    "2025-04-15-online_safety_guide_urdu_08-03-2024.pdf",
]


def clean_text(text: str) -> str:
    text = re.sub(r"[ \t]+", " ", text)   # collapse horizontal whitespace only
    text = re.sub(r"\n{2,}", "\n", text)   # collapse repeated newlines
    text = text.strip()
    return text


def detect_language(text: str) -> str:
    if len(text) < 20:
        return "unknown"
    try:
        lang = detect(text)
        if lang in ("ur", "en"):
            return lang
        return "other"
    except Exception:
        return "unknown"


def extract_pdf(pdf_path: Path) -> list[dict]:
    records = []
    try:
        doc = fitz.open(pdf_path)
        for page_num in range(len(doc)):
            raw_text = doc[page_num].get_text()
            cleaned = clean_text(raw_text)
            if len(cleaned) < 30:
                continue
            language = detect_language(cleaned)
            records.append({
                "source": pdf_path.name,
                "page": page_num + 1,
                "language": language,
                "text": cleaned
            })
        doc.close()
    except Exception as e:
        print(f"  ERROR processing {pdf_path.name}: {e}")
    return records


def main():
    all_records = []
    pdf_files = [p for p in RAW_DIR.rglob("*.pdf") if p.name not in SKIP_NATIVE_EXTRACTION]

    if not pdf_files:
        print(f"No PDFs found in {RAW_DIR}.")
        return

    print(f"Found {len(pdf_files)} PDF(s) for native extraction (scanned files handled separately).\n")

    for pdf_path in pdf_files:
        print(f"Processing: {pdf_path.name}")
        records = extract_pdf(pdf_path)
        all_records.extend(records)

        ur_count = sum(1 for r in records if r["language"] == "ur")
        en_count = sum(1 for r in records if r["language"] == "en")
        other_count = len(records) - ur_count - en_count
        print(f"  -> {len(records)} pages extracted (ur: {ur_count}, en: {en_count}, other/unknown: {other_count})")

    out_path = OUT_DIR / "corpus.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(all_records, f, ensure_ascii=False, indent=2)

    total_ur = sum(1 for r in all_records if r["language"] == "ur")
    total_en = sum(1 for r in all_records if r["language"] == "en")
    print(f"\nDone. {len(all_records)} total page-records saved to {out_path}")
    print(f"Language breakdown -> Urdu: {total_ur}, English: {total_en}, Other/Unknown: {len(all_records) - total_ur - total_en}")
    print(f"\nNote: {len(SKIP_NATIVE_EXTRACTION)} scanned file(s) skipped here - run ocr_fix.py next to add them via OCR.")


if __name__ == "__main__":
    main()
